 ...
 */
/* package */
internal class Http2SniffInterceptor     /* package */(private val mCallback: SSLRefluxCallback<HttpRequest, HttpResponse>) :
    ...
    override fun intercept(
        ...
        if (index == 0) {
            ...
            if (request!!.isHttps) {
                if (buffer.hasRemaining() && Bytes.indexOf(
                        buffer.array(),
                    ...
                    if (buffer.remaining() == Http2.CONNECTION_PREFACE.size) {
                        // Skip preface frame data.
                        mCallback.onRequest(request, buffer)
                        return
                    } else {
                        val prefaceBuffer = ByteBuffer.allocate(Http2.CONNECTION_PREFACE.size)
                        prefaceBuffer.put(Http2.CONNECTION_PREFACE)
                        prefaceBuffer.flip()
                        mCallback.onRequest(request, prefaceBuffer)
                        // The remaining data continues.
                        buffer.position(buffer.position() + Http2.CONNECTION_PREFACE.size)
                    }
                }
            }
        }

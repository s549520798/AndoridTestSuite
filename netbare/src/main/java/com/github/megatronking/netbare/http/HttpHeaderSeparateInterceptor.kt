 ...
 */
/* package */
internal class HttpHeaderSeparateInterceptor : HttpPendingIndexedInterceptor() {
    private var mRequestHeaderHandled = false
    private var mResponseHeaderHandled = false
    ...
    override fun intercept(
        ...
        val headerEndIndex = Bytes.indexOf(buffer.array(), NetBareUtils.PART_END_BYTES)
        if (headerEndIndex < 0) {
            ...
            val hasMultiPart = headerEndIndex < buffer.limit() - NetBareUtils.PART_END_BYTES.size
            if (hasMultiPart) {
                mLog!!.w("Multiple http request parts are founded.")
                // Separate the header and body data to two buffers.
                val offset = headerEndIndex + NetBareUtils.PART_END_BYTES.size
                val headerBuffer = ByteBuffer.wrap(buffer.array(), buffer.position(), offset)
                // Allocate a new buffer, do not use wrap, different buffers will share the same array.
                val bodyBuffer = ByteBuffer.allocate(buffer.limit() - offset)
    ...
    @Throws(IOException::class)
    override fun intercept(
        @NonNull chain: HttpResponseChain,
        @NonNull buffer: ByteBuffer,
        index: Int
    ) {
        var buffer: ByteBuffer? = buffer

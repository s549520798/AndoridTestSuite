 ...
 */
class TcpProxyServerForwarder(
    vpnService: VpnService, ip: String?, mtu: Int,
    dumper: UidDumper?
) : ProxyServerForwarder {
    private val mSessionProvider: SessionProvider
    private val mProxyServer: TcpProxyServer
    ...
    override fun forward(packet: ByteArray, len: Int, output: OutputStream) {
        ...
        if (localPort != mProxyServer.port) {
            ...
            ipHeader.destinationIp = mProxyServer.ip
            tcpHeader.destinationPort = mProxyServer.port
            ...
            val session = mSessionProvider.query(remotePort)
            if (session == null) {
    ...
    init {
        ...
        mProxyServer = TcpProxyServer(vpnService, ip, mtu)

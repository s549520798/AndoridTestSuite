 ...
 */
class CertificateInstallActivity : Activity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent) {
        ...
        if (requestCode == REQUEST_CODE_INSTALL && resultCode == RESULT_OK) {
            val jsk = File(
                cacheDir,
                intent.getStringExtra(EXTRA_ALIAS) + JKS.Companion.KEY_JKS_FILE_EXTENSION
            )
            try {
                if (!jsk.exists() && !jsk.createNewFile()) {
                    throw IOException("Create jks file failed.")
                }
            } catch (e: IOException) {
                NetBareLog.wtf(e)
            }
    ...
    companion object {
        ...
        const val EXTRA_ALIAS = "jks_alias"
    }
}
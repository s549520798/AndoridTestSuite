 ...
 */
class UdpRemoteTunnel(
    private val mVpnService: VpnService, channel: DatagramChannel, selector: Selector,
    remoteIp: String?, remotePort: Short
) : UdpTunnel(channel, selector) {
    ...
    override fun connect(address: InetSocketAddress) {
        if (mVpnService.protect(socket())) {
            ...
            throw IOException("[UDP]Can not protect remote tunnel socket.")
        }
    }
    ...
    init {
        mLog = NetBareXLog(Protocol.UDP, remoteIp, remotePort)
    }
}